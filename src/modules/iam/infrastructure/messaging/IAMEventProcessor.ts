...
export class IAMEventProcessor {
  ...
  async process(eventType: string, entityType: string, entityId: string, tenantId: string, data: Record<string, any>): Promise<void> {
    try {
      ...
      this.logger.debug(`[IAM] Processed IAM event: ${eventType} for ${entityType}:${entityId}`, this.context);
      
    } catch (error) {
      this.logger.error(`[IAM] ✗ IAM event processing failed: ${error.message}`, error.stack, this.context);
      throw error;
    }
  }
  ...
  private async processIAMEvent(
    ...
    switch (eventType) {
      ...
      case 'PermissionRevoked':
        ...
        break;
      default:
        this.logger.debug(`[IAM] Unhandled IAM event type: ${eventType}`, this.context);
    }
  }
  ...
  private async handleUserEvent(eventType: string, userId: string, tenantId: string, data: any): Promise<void> {
    ...
    this.logger.debug(`[IAM] Processing user event: ${eventType} for user ${userId}`, this.context);
  }
  ...
  private async handleRoleEvent(eventType: string, roleId: string, tenantId: string, data: any): Promise<void> {
    ...
    this.logger.debug(`[IAM] Processing role event: ${eventType} for role ${roleId}`, this.context);
  }
  ...
  private async handlePermissionEvent(eventType: string, permissionId: string, tenantId: string, data: any): Promise<void> {
    ...
    this.logger.debug(`[IAM] Processing permission event: ${eventType} for permission ${permissionId}`, this.context);
  }
}